    ...
    def generate(
        ...
        with torch.cuda.amp.autocast():
            ...
            for i in range(len(abnormal_feats1)):
                ...
                cls_pred_2 = 1 if abnormal_feats2[i][1] >= abnormal_feats2[i][0] else 0
                cls_pred_3 = 1 if abnormal_feats3[i][1] >= abnormal_feats3[i][0] else 0
                cls_pred_4 = 1 if abnormal_feats4[i][1] >= abnormal_feats4[i][0] else 0
                cls_pred_5 = 1 if abnormal_feats5[i][1] >= abnormal_feats5[i][0] else 0
                cls_pred_6 = 1 if abnormal_feats6[i][1] >= abnormal_feats6[i][0] else 0
...
def available_models():
    return ["VisionUnite V1 public checkpoint"]


def load(name, llama_dir, device="cuda" if torch.cuda.is_available() else "cpu", knn=False):
    if not os.path.isfile(name):
        return RuntimeError(f"Model checkpoint not found: {name}")
    model_path = name
    ...
    adapter_ckpt = torch.load(model_path, map_location='cpu', weights_only=False, mmap=True)
    checkpoint_state = adapter_ckpt['model']
    del adapter_ckpt
    ...
    missing_parameters = sorted(key for key in load_result.missing_keys if key in parameter_names)
    if missing_parameters:
        preview = ", ".join(missing_parameters[:12])
        raise RuntimeError(
            f"VisionUnite checkpoint is not self-contained: {len(missing_parameters)} "
            f"parameter tensors are missing ({preview})"
        )
